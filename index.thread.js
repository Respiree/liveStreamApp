...
self.onmessage = (message) => {
 ...
 let {read_data, params, sqa_bandpass_rr} = obj;
 let res = calculate_rr(read_data, params, sqa_bandpass_rr);
...
function moving_average(time, value, window_size){
    // console.log("moving average called")
    if (time.length != value.length) {
        throw new Error('length of time column does not match length of value column.')
    }
    if (window_size > value.length) {
        throw new Error('window size is greater than number of elements.')
    }
    ...
    if (window_size == 0) {
        ...
        // initialize moving sum 
        let moving_sum = 0;
        for (i=0; i<window_size; i++) {
            moving_sum += value[i];
        }
        for (i=0; i<len-window_size+1; i++) {
            let moving_average = moving_sum/window_size
            let moving_time = time[i+Math.floor(window_size/2)]
            mva_value.push(moving_average)
            mva_time.push(moving_time)
            moving_sum -= value[i]
            moving_sum += value[i+window_size]
        }
    }
...
function median(window_size){
    function get(arr){
        arr.sort((a,b)=>a-b)
        if(window_size%2){
            return arr[Math.floor(window_size/2)]
        } else {
...
function baseline(time, value, window_size){
    // console.log("baseline function called")
    if (time.length != value.length) {
        throw new Error('length of time column does not match length of value column.')
    }
    if (window_size > value.length) {
    ...
    if (window_size == 0) {
        ...
        let moving_values = [];
        for (i=0; i<window_size; i++) {
            moving_values.push(value[i]);
        }
        // different formula for window_size even or odd. 
        let get_median = median(window_size)
        for (i=0; i<len-window_size; i++) {
            let baseline_value = get_median(moving_values)
            let moving_time = time[i+Math.floor(window_size/2)]
            bl_value.push(baseline_value)
            bl_time.push(moving_time)
            moving_values.splice(moving_values.indexOf(value[i]),1)
            moving_values.push(value[i+window_size])
        }
    }
...
function artifact(time,value,window_size,mva_filt=true){
    // console.log("artifact function called")
    if (time.length != value.length) {
        throw new Error('length of time column does not match length of value column.')
    }
    if (window_size > value.length) {
    ...
    for (i=1; i<len-1; i++) {
        art_output.push(Math.abs(value[i]**2 - value[i-1]*value[i+1]))
        art_time.push(time[i])
    }

    if (mva_filt==true){
        let ma = moving_average(art_time, art_output, window_size)
        art_time = ma.mva_time
        art_output = ma.mva_value
    }
...
function baseline_removal(signal_time,signal_value,baseline_time, baseline_value){
    // console.log("baseline_removal function called")
    if (signal_time.length != signal_value.length) {
        throw new Error('inconsistent column lengths for signal.')
    }
    if (baseline_time.length != baseline_value.length) {
        throw new Error('inconsistent length column for baseline.')
    }
    if (signal_time.indexOf(baseline_time[0])==-1){
        throw new Error('start of baseline_time not found in signal_time.')
    }
    if (signal_time.indexOf(baseline_time[baseline_time.length-1])==-1){
        throw new Error('end of baseline_time not found in signal_time.')
    }
    if (signal_time.indexOf(baseline_time[baseline_time.length-1])-signal_time.indexOf(baseline_time[0])+1 != baseline_time.length){
        throw new Error('number of elements in baseline_time different from signal_time')
    }

    let blrm_time = baseline_time;
    let blrm_value = []

    bl_offset = signal_time.indexOf(baseline_time[0])
    bl_length = baseline_time.length
    for (i=0; i<bl_length; i++) {
        blrm_value.push(signal_value[bl_offset+i]-baseline_value[i])
    }
    return { 
        blrm_time: blrm_time,
        blrm_value: blrm_value
    }
}

function artifact_removal(signal_time,signal_value,art_time,art_value,artbl_time,artbl_value,th,window_size,mva_filt=true){
    // console.log("artifact_removal function called")
    if (signal_time.length != signal_value.length) {
        throw new Error('inconsistent column lengths for signal.')
    }
    if (art_time.length != art_value.length) {
        throw new Error('inconsistent column lengthw for art.')
    }
    if (artbl_time.length != artbl_value.length) {
        throw new Error('inconsistent column lengths for artbl.')
    }

    // signal_val = signal_val[signal_time.index(artbl_time[0]):signal_time.index(artbl_time[-1])+1]  
    if (signal_time.indexOf(artbl_time[0])==-1){
        throw new Error('start of artbl_time not found in signal_time.')
    }
    if (signal_time.indexOf(artbl_time[artbl_time.length-1])==-1){
        throw new Error('end of artbl_time not found in signal_time.')
    }
    if (signal_time.indexOf(artbl_time[artbl_time.length-1])-signal_time.indexOf(artbl_time[0])+1 != artbl_time.length){
        throw new Error('number of elements in artbl_time different from signal_time')
    }

    let artbl_offset = signal_time.indexOf(artbl_time[0])
    let artbl_length = artbl_time.length

    let signal_value_ = []
    for (i=0; i<artbl_length; i++) {
        signal_value_.push(signal_value[artbl_offset+i])
    }
    signal_value = signal_value_; 

    // art_val = art_val[art_time.index(artbl_time[0]):art_time.index(artbl_time[-1])+1]
    if (art_time.indexOf(artbl_time[0])==-1){
        throw new Error('start of artbl_time not found in art_time.')
    }
    if (art_time.indexOf(artbl_time[artbl_time.length-1])==-1){
        throw new Error('end of artbl_time not found in art_time.')
    }
    if (art_time.indexOf(artbl_time[artbl_time.length-1])-art_time.indexOf(artbl_time[0])+1 != artbl_time.length){
        throw new Error('number of elements in artbl_time different from art_time')
    }

    let art_value_ = []
    for (i=0; i<artbl_length; i++) {
        art_value_.push(art_value[artbl_offset+i])
    }
    art_value = art_value_; 

    // output = signal_val[art_val<th*artbl_val]
    let output_ = []
    let time_ = []
    for (i=0; i<artbl_length; i++){
        let s = signal_value_[i];
        if(art_value_[i]<th*artbl_value[i]){
            output_.push(s)
            time_.push(artbl_time[i])
        }
    }
    ...
    let output 
    if (mva_filt==true){
        let ma = moving_average(time_, output_, window_size)
        time = ma.mva_time
        output = ma.mva_value
    } else {
        time = time_
...
function metric_extraction(params, signal, coef_A, coef_B, plag, sampling_time, m1_min, m1_max){
    ...
    // init cooefficients
    const lag = plag || 5
    const threshold = coef_A || 3.5
    const influence = coef_B || 0.5
...
function map_input_data(blrm_value, blrm_time, art_value, art_time, artbl_value, artbl_time){
    if (blrm_value.length != blrm_time.length) {
        throw new Error('inconsistent column lengths for blrm.')
    }
    if (art_value.length != art_time.length) {
        throw new Error('inconsistent column lengthw for art.')
    }
    if (artbl_value.length != artbl_time.length) {
        throw new Error('inconsistent column lengths for artbl.')
    }
    if (blrm_time.indexOf(artbl_time[0])==-1){
        throw new Error('start of artbl_time not found in blrm_time.')
    }
    if (blrm_time.indexOf(artbl_time[artbl_time.length-1])==-1){
        throw new Error('end of artbl_time not found in blrm_time.')
    }
    if (blrm_time.indexOf(artbl_time[artbl_time.length-1])-blrm_time.indexOf(artbl_time[0])+1 != artbl_time.length){
        throw new Error('number of elements in artbl_time different from blrm_time')
    }
    if (art_time.indexOf(artbl_time[0])==-1){
        throw new Error('start of artbl_time not found in art_time.')
    }
    ...
    blrm_offset = blrm_time.indexOf(artbl_time[0])
    ks_offset = art_time.indexOf(artbl_time[0])
    ...
    for (i=0; i<=artbl_time.length; i++){
        // signal.push([blrm_time[i+blrm_offset], blrm_value[i+blrm_offset]])
        // art.push([art_time[i+ks_offset], art_value[i+ks_offset]])
        // artbl.push([artbl_time[i], artbl_value[i]])
        X_signal_time.push(blrm_time[i+blrm_offset])
        X_signal_value.push(blrm_value[i+blrm_offset])
        X_art_time.push(art_time[i+ks_offset]) 
        X_art_value.push(art_value[i+ks_offset])
        X_artbl_time.push(artbl_time[i]) 
        X_artbl_value.push(artbl_value[i])
    }
    ...
    // }
    return {
        X_signal_time: X_signal_time,
        X_signal_value: X_signal_value,
        X_art_time: X_art_time,
        X_art_value: X_art_value,
        X_artbl_time: X_artbl_time,
        X_artbl_value: X_artbl_value
    }
}
function hanning(M){
    ...
    let h = []
    for (i=0; i<M; i++){
        h.push(0.5-0.5*Math.cos(2*Math.PI*i/(M-1)))
    }
    return h
}

// function run_fft(Xin, fs, n_fft=2**12){
function run_fft(Xin, fs){
    let h = hanning(Xin.length)
    let X = []
    for (i=0;i<Xin.length;i++){
        X[i] = Xin[i]*h[i]
    }

    let two_exponent = Math.ceil(Math.log(Xin.length)/Math.log(2))
    for (i=Xin.length; i < 2**two_exponent; i++){
        X[i] = 0
    }
    // this is the total period fed into the fft. 
    let T = (2**two_exponent)/fs; 
    fft = new (new DSP_LIB()).FFT(2**two_exponent, fs);
    fft.forward(X);
    let yf = Array.from(fft.spectrum);

    let x_freq=[]
    for (i=0;i<yf.length;i++){
        x_freq.push(i/T) // in Hz. 
    }
...
export function calculate_rr(read_data, params, sqa_bandpass_rr){
    ...
    let bl = baseline(mva.mva_time, mva.mva_value, params['window_bl'])
    let blrm = baseline_removal(mva.mva_time, mva.mva_value, bl.bl_time, bl.bl_value)
    let art = artifact(blrm.blrm_time, blrm.blrm_value, params['window_art_mva'])
    let artbl = baseline(art.time, art.output, params['window_art_bl'])



    // # get_inputs_for_map_input_data
    map_input_data_results = map_input_data(
        blrm["blrm_value"], blrm["blrm_time"], 
        art["output"], art["time"], 
        artbl["bl_value"], artbl["bl_time"]
    )
    let X_signal_t = map_input_data_results["X_signal_time"]
    let X_signal_v = map_input_data_results["X_signal_value"]
    let X_art_t = map_input_data_results["X_art_time"]
    let X_art_v = map_input_data_results["X_art_value"]
    let X_artbl_t = map_input_data_results["X_artbl_time"]
    let X_artbl_v = map_input_data_results["X_artbl_value"]
    ...
    let save_time = [] //# timestamp of each extraction
    let save_bpm = []
    let save_bpm_round = []
    ...
    let n_window = 0
    for (let i=0; i<(X_signal_t.length-window_size); i+=window_shift) {

        ws = artifact_removal(
            X_signal_t.slice(i,i+window_size-1), X_signal_v.slice(i,i+window_size-1),
            X_art_t.slice(i,i+window_size-1), X_art_v.slice(i,i+window_size-1),
            X_artbl_t.slice(i,i+window_size-1), X_artbl_v.slice(i,i+window_size-1),
            params['th_prepro'], params['window_smooth_output'], 
            true
        )
        window_signal = ws["output"]

  
        if (window_signal.length>=params['lag']){
        
            // # get_inputs_for_metric_extraction
            trinary = metric_extraction(
                params,
                window_signal,
                params['coef_A'], params['coef_B'],
        ...
        save_time.push(X_signal_t[i+window_size]) //# timestamp of each extraction
        ...
        temp_trinary = (function remove_mean(t){
            let sum = 0;
            for (let j=0; j<t.length; j++){
                sum += t[j]
            }
            let mean = sum/t.length;
            let t_ = []
        ...
        run_fft_results = run_fft(temp_trinary, 1/params['sampling_time'])
    

        // bp_and_select_freq_result = bp_and_select_freq(run_fft_results['freq_wo_bp'], run_fft_results['mag_wo_bp'],sqa_bandpass_rr)
        bp_and_select_freq_result = (function bp_and_select_freq(freq_wo_bp, mag_wo_bp, bandpass){
            let i = 0;
            let freq_bp = []
            let mag_bp = []
            let max_mag = 0
            let idx_max_peak = null
            while (freq_wo_bp[i]<=bandpass[1]){
                if(freq_wo_bp[i]>=bandpass[0]){
                    if(mag_wo_bp[i]>max_mag){
                        max_mag = mag_wo_bp[i]
                        idx_max_peak = i
                    }
                    freq_bp.push(freq_wo_bp[i])
                    mag_bp.push(mag_wo_bp[i])
                }
                i++;
            }

            return {
                selected_freq: freq_wo_bp[idx_max_peak],
                freq_bp: freq_bp,
                mag_bp: mag_bp,
            }
        })(run_fft_results['freq_wo_bp'], run_fft_results['mag_wo_bp'],sqa_bandpass_rr)

        let selected_freq = bp_and_select_freq_result["selected_freq"]


        // """
        // convert to BPM
        // """
        let temp_bpm = 60*selected_freq;

        // NOT HANDLED YET - if np.isnan(temp_bpm): 
        // although Math.round(NaN) = NaN
        // still have to be careful of edge scenerios like [NaN]
        let temp_bpm_round = Math.round(temp_bpm);

        save_bpm_round.push(temp_bpm_round)
        n_window+=1
    }
    console.log('RESULT',  save_bpm_round[0])
    return save_bpm_round[0];
}