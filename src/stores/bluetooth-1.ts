...
class BleStore extends HydratedStore {
      ...
      if(this.gatewayMode) {
          ...
          if(this.record.recordReceivedByGateway < this.sensorRecords) {
            ...
            if(remaingRecords < 1000){
              ...
              this.saveRecord();
              this.clearCurrRecords();
              this.callbackToAllListener(null, {type: SuccessType.Ble_Syncing_Record, remaingRecords})
              this.postCount++;
              const thisble = this;
              if(mBleTimer) {
  ...
  handleUpdateValueForCharacteristic(data:any) {
    ...
    if(data.value[0] == BleSensorConstant.BLE_SENSOR_COMMAND_START_BYTE && data.value[1] == BleSensorConstant.BLE_SENSOR_COMMAND_RES_SEND_ALL_RECORDS){ //Send record response
      if(data.value[4] == BleSensorConstant.BLE_SENSOR_ERROR_SEND_ALL_RECORDS_FAILED){
        printOut("no new record");
        this.callbackToAllListener(null, {type:SuccessType.Ble_Syncing_Record, noRecords:0})
      }
      else{
          //printOut('value---->>>', data.value)
        ...
        printOut("noRecords " + noRecords + ", battery: " + batteryLevel + ", fwVer:" + fwVersion);
      ...
      if(value[14] == 0 && value[15] == 0 && value[16] == 0 && value[17] == 0) {
          ...
          const dataOffset = mBleSensorPackedFormat ? 10 : 16;
          ...
          do {
              const mode = value[1];
              const temp = (value[2] << 8) | (value[3]);
              const sensor1 = (value[4] << 8) | (value[5]);
              const sensor2 = (value[6] << 8) | (value[7]);
              const axl_x = (value[8] << 8) | (value[9]);
              const axl_y = (value[10] << 8) | (value[11]);
              const axl_z = (value[12] << 8) | (value[13]);
              const timestamp = new Uint8Array([value[14],value[15], value[16], value[17]]);
              ...
              this.record!.battery = this.battery;
              ...
              remaining_length -= dataOffset;

          } while(remainingLength > dataOffset);

          //console.log("receive",this.record);
          this.callbackToAllListener(null, {type:SuccessType.Ble_Received_Data, receive: this.currRecordCount})
      }
  ...
  @action setMode = async (mode:boolean) => {
  ...
  @action sendAllRecords = (deviceId: string, patientId: number) =>{
    ...
    let pbytes = [];
    pbytes[0] = BleSensorConstant.BLE_SENSOR_COMMAND_START_BYTE;
    pbytes[1] = BleSensorConstant.BLE_SENSOR_COMMAND_REQ_SEND_ALL_RECORDS;
    pbytes[2] = ((patientId >> 8) & 0xff);
    pbytes[3] = (patientId & 0xff);
    var idx = 4;
    var len = 15;
