...
import { BleCallback, convertToHexString, ErrType, numberToString, SuccessType } from 'src/stores/bluetooth';
import {
  useQuery,
  useQueryClient,
  QueryClient,
...
const Component: NavigationFunctionComponent = observer(({
  ...
  const [countdownTime, setCountdownTime] = useState(0);//mGatewayModeCountdownSec);
  const [isTimerTriggered, setIsTimerTriggered] = useState(false);
  const [isSecTick, setIsSecTick] = useState(false);
  const [refetchInterval, setRefetchInterval] = useState(-1);//mGatewayModeCountdownSec * 1000);
      ...
      if(result){
        ...
        if(result === SuccessType.Ble_State_Changed){
          ...
          if(bluetooth.connected && bluetooth.connectedId){
            ...
            }else if(bluetooth.gatewayMode && patient.id && patient.isRegister && !bluetooth.isBusy){
          ...
          if(result.data && result.data.value){
            ...
            if(value[0] == bluetooth.BLE_SENSOR_COMMAND_START_BYTE && value[1] == bluetooth.BLE_SENSOR_COMMAND_RES_GET_DEVICE_STATUS){ //get device status response
              console.log("get device status response",value)
              if(value[12] == bluetooth.BLE_SENSOR_RESPONSE_SUCCESS){
  ...
  const connectDevice = (enableAlert=true, queryStatus=true)=>{
    ...
    if(patient.sensors.length>0 && patient.sensors[0].id){
      if(bluetooth.isOn && bluetooth.connected && bluetooth.connectedId == patient.sensors[0].id)
        return;
        console.log("connect device",connectedDevice);
      if(connectedDevice !== "")
        return false;
      //nav.showSpinner();
  ...
  useEffect(() => {
    ...
    console.log("first check on gateway mode")
    if(bluetooth.gatewayMode && patient.isRegister) {
        if(!isGatewayMode) {
  ...
  const getDashboardData = async()=>{
    ...
    const userIdStr = numberToString(userId);
  ...
  const { isFetching } = useQuery("userData", () =>
    {
      console.log("timeout to send record if any","mode",bluetooth.gatewayMode, "isRegistered",patient.isRegister);

      if(bluetooth.gatewayMode && patient.isRegister){
        if(bluetooth.savedRecords && bluetooth.savedRecords.recordCollectedBySensor > 0){
          //there are records saved
          console.log("upload to server first");
          uploadToServer();
        }else{
          console.log("connect device");

          if(connectDevice(false, false)) {
    ...
    ,{
      refetchInterval: refetchInterval,//mGatewayModeCountdownSec * 1000,//3*60000,
      //enabled: true
    }
  );
    ...
    if(bluetooth.gatewayMode && patient.isRegister) {
        ...
        if(mSecTimer == null) {
            mSecTimer = setInterval(() => {
                //console.log("setIsSecTick: " + countdownTime);
                setIsSecTick(true);
            }, 1000);
        }
        if(!isGatewayMode) {
        ...
        <Text style={styles.date}>{`${t.do("today").toUpperCase()}, ${moment().format('DD MMMM YYYY').toUpperCase()}`}</Text>
        {
          ...
          (<View style={[styles.batteryView, generateShadow({elevation:1.5})]}>
            ...
            <View style={styles.batteryLevels}>
              <View style={battery<40?styles.batteryRed:styles.batteryGreen}/>
              <View style={battery>=40?styles.batteryGreen:styles.batteryGrey}/>
              <View style={battery>=60?styles.batteryGreen:styles.batteryGrey}/>
              <View style={battery>=80?styles.batteryGreen:styles.batteryGrey}/>
              <View style={battery>=100?styles.batteryGreen:styles.batteryGrey}/>
            </View>
          </View>):
          (<View style={styles.connectView}>
            ...
            <View style={[ bluetooth.gatewayMode && patient.isRegister ? styles.buttonDisabledView : styles.buttonView, generateShadow({elevation:1.5, shadowColor:'rgba(202, 207, 235, 0.3)'})]}>
                <ButtonTitle textStyle={styles.buttonText} title={t.do('connect').toUpperCase()} onPress={connectDevice}/>
            </View>
          </View>)
        }
        {updateChart?
