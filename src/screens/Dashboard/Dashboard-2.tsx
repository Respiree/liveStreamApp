...
const Component: NavigationFunctionComponent = observer(({
  ...
  /*if(mSecTimer == null) {
      mSecTimer = BackgroundTimer.setInterval(() => {
  ...
  const { isStale, isSuccess, status } = useQuery("userData", () =>
    {
      ...
      if(bluetooth.gatewayMode && patient.isRegister /*&& countdownTime == 0*/ && refetchInterval != -1) {// && (countdownTime == 0 || refetchInterval == (mGatewayModeCountdownSec* 1000))){
        globalCountDown = -1;
        setCountdownTime(-1);
        console.log("connect device==================")
