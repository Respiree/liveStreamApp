...
const Component: NavigationFunctionComponent = observer(({
  ...
  const { colors, sizes ,options, apiUrl } = useConstants();
        ...
        if(result === SuccessType.Ble_State_Changed){
          ...
          if(!bluetooth.connected){
            ...
            if(bluetooth.gatewayMode && patient.isRegister) {
                ...
                enableBackgroundTimer(mGatewayModeCountdownSec * 1000);
            }
          }
        }
  ...
  const handleAppStateChange = (state: any) => {
    //console.log("app state change!!!!!!!!",Platform.OS, state, "interval",refetchInterval, "suspend",isSuspend);
    if(state === "inactive"){
      ble.saveRecord();
      ble.uninit();
  ...
  useEffect(() => {
    ...
    //return () => bluetooth.removeBluetoothCallback(componentId);
  },[]);
  ...
  const getDashboardData = async()=>{
    ...
    const url = `${apiUrl.dashboard}?id=${userIdStr}&date_time=${today}`
    console.log("url===",url);
    fetch(url, {
         method: 'GET'
      })
  ...
  /*const { isStale, isSuccess, status } = useQuery("userData", () =>
    ...
    ,{
      ...
      enabled: false//!isSuspend
    }
  );*/

  const enableBackgroundTimer = (timerInterval) => {
      if(mBackgroundTimerId) {
          BackgroundTimer.clearInterval(mBackgroundTimerId);
          mBackgroundTimerId = null;
      }

      console.log("enableBackgroundTimer>>>>>>>>>>>> " + timerInterval)
      mGlobalTimer = new Date();
      mBackgroundTimerId = BackgroundTimer.setInterval(() => {
          console.log("enableBackgroundTimer>>>>>>>>>>>> called")
          if(bluetooth.gatewayMode && patient.isRegister) {// && refetchInterval != -1) {
            globalCountDown = -1;
            setCountdownTime(-1);
            console.log("connect device==================")
...
const _styles = (theme: ThemeType) => ScaledSheet.create({
  ...
  batteryView:{
    ...
    borderColor:theme.colors.very_light_grey,
    borderWidth:1,
    alignItems:'center',
    justifyContent:'space-between',
