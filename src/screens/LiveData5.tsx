...
const MAX_RECORDS = MIN_RECORDS * 20;
...
const LiveData: React.FC = observer(({
      ...
      if(result){
        if(result === SuccessType.Ble_State_Changed){
          ...
          if(result.sensor1 >= MIN_SENSOR_VALUE ) {
              if(sensor1.length > MAX_RECORDS+MOVING_AVERAGE_SIZE) sensor1.shift();
              sensor1.push(result.sensor1);
          }
          if(result.sensor2 >= MIN_SENSOR_VALUE) {
              if(sensor2.length > MAX_RECORDS+MOVING_AVERAGE_SIZE) sensor2.shift();
  ...
  useEffect(()=>{
      if(isAddData) {
          ...
          if(sensor1.length > 0 && isSensor1On){
              let sliceLen = sensor1.length > MAX_RECORDS+MOVING_AVERAGE_SIZE ? MAX_RECORDS+MOVING_AVERAGE_SIZE : sensor1.length;
          ...
          if(sensor2.length > 0 && isSensor2On) {
              let sliceLen = sensor2.length > MAX_RECORDS+MOVING_AVERAGE_SIZE ? MAX_RECORDS+MOVING_AVERAGE_SIZE : sensor2.length;
  ...
  const fillArray = (arrayLen, values) => {
      ...
      for(ii = 0; ii < copyLen; ii++) {
          ...
          if(ii == MOVING_AVERAGE_SIZE) {
              //console.log("last values>>> ", lastValues)
          }
          if(ii == MOVING_AVERAGE_SIZE+1) {
              //console.log("last values +1>>> ", lastValues)
          }
          if(ii >= MOVING_AVERAGE_SIZE-1) {
